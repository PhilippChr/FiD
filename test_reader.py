		...
		for i, batch in enumerate(dataloader):
			...
			if opt.top_k > 1:
				if torch.cuda.is_available():
					outputs = model.generate(
						...
						num_return_sequences=opt.top_k
					)
				else:
					outputs = model.generate(
			...
			if opt.top_k > 1:
				...
				for k, o in enumerate(outputs):
					answers = list()
					for o_beam_idx, o_beam in enumerate(o):
						ans = tokenizer.decode(o_beam, skip_special_tokens=True, clean_up_tokenization_spaces=True)
						answers.append(ans)
					example = dataset.data[idx[k]]
					if "answers" in example:
						score = src.evaluation.ems(answers[0], example["answers"])
						exactmatch.append(score)

					if opt.write_results:
						answers_string = "||".join(answers)
						fw.write(str(example["id"]) + "\t" + answers_string + "\n")
					if opt.write_crossattention_scores:
...
if __name__ == "__main__":
	...
	collator_function = src.data.Collator(opt.text_maxlength, tokenizer, logger)
